...
and ac.company_code = :company_code
...
select %[1]s.seq_standinginstruction.nextval as id from dual

-- name: GetIDRecurrentTransaction-main
select %[1]s.seq_recurrenttransaction.nextval as id from dual
...
select rl.nomor_rekening ,rt.nama_rekening ,rt.keterangan ,rl.nomor_nasabah ,n.jenis_nasabah ,to_char(rl.tanggal_buka ,'YYYY-MM-DD') as tanggal_buka
			...
			,rt.saldo - (nvl(rl.saldo_float, 0.0) + nvl(rl.saldo_ditahan, 0.0) + nvl(p.saldo_minimum, 0.0)) as saldo_efektif, rl.is_blokir_debet ,rl.is_blokir_kredit 
...
select rt.nama_rekening,  p.kode_produk, p.nama_produk, concat(concat(rt.kode_cabang, '-'), c.nama_cabang) as cabang, 
			...
			nvl(rl.is_tutup_kartu_atm, 'F') as is_tutup_kartu_atm
		from %[1]s.rekeningliabilitas rl
...
and a.Kode_Cabang = :kode_cabang

-- name: GetPendingClrDepositList-filter-Tanggal
and a.Tanggal_Efektif = to_date(:tanggal_efektif, 'yyyy-mm-dd')
...
select %[1]s.seq_validasi_reposisi_rekening.nextval from dual
...
SELECT %[1]s.seq_validasi_reposisi_rekening.nextval
...
INSERT INTO %[1]v.INFOREPOSISIREKENING (id_transaksi, tanggal, nomor_Rekening, cabang_asal, cabang_tujuan, user_input, user_otorisasi, cabang_input, saldo_reposisi, kode_produk)
		VALUES(
			%[1]v.seq_transaksi.nextval, current_timestamp, :NOMOR_REKENING, :KODE_CABANG, :KODE_CABANG_TUJUAN, :USER_INPUT, :USER_OTORISASI, :KODE_CABANG_INPUT, :SALDO_REPOSISI, :KODE_PRODUK
		)
...
SELECT %[1]s.seq_val_ubahproduk_rekening.nextval  FROM dual
...
select nvl(sum(amount_credit - amount_debit) * aci.balance_sign,0.0) as total_today
		from %[1]s.journal j 
...
insert into %[1]s.histblokir
		...
		values(%[1]s.seq_histblokir.nextval, :NOMOR_REKENING, :KETERANGAN_BLOKIR, :USER_INPUT, :USER_DUALCONTROL, :NOMOR_NASABAH, :NAMA_LENGKAP, :STATUS_BLOKIR, 
			:TANGGAL_UBAH, :SISTEM_PEMBLOKIR, :KODE_CABANG_INPUT, :KODE_CABANG_PEMBLOKIR)
...
and m.status_peserta_skn = :status_peserta

-- name: GetKodeBankTrfList-filter-JenisMemberRTGS
and m.status_peserta_rtgs = :status_peserta
...
select dbms_lob.getlength(data) blob_data_length
		from %[1]s.blobdata
		where id_data = :ID_DATA

-- name: GetBlobData-main
select dbms_lob.substr(data, 3500, :OFFSET) blob_data
		from %[1]s.blobdata
		where id_data = :ID_DATA

-- name: GetBlobdataId-main
select %[1]s.seq_blobdata.nextval
		from dual
...
insert into %v.holddanarekening
		...
		values (%v.seq_holddanarekening.nextval, :STATUS, :SISTEM_HOLD, :NOMOR_REKENING, :NOMINAL_HOLD, :ALASAN_HOLD, :TANGGAL_HOLD, 
...
update  %v.rekeningliabilitas rl 
		set saldo_ditahan = (
			select sum(nvl(h.nominal_hold,0.0)) saldo_hold
			from %v.holddanarekening h
			where h.status = 'A'
				and h.nomor_rekening = :nomor_rekening
		)
...
select 
			...
			nvl(is_regulatory,'F') as is_regulatory
...
select %[1]s.seq_cardcustomerrequest.nextval as id_request from dual
...
insert into %[1]s.historycardstatus
		...
		values(%[1]s.seq_historycardstatus.nextval, :CARDID, :CUSTOMERNAME, :CUSTOMERNO, :CARDSTATE_BEFORE, :CARDSTATE_AFTER, current_timestamp, :DESCRIPTION, :USER_CHANGE)
...
select %[1]s.seq_bukuwarkat.nextval as id_bukuwarkat from dual
...
and (upper(b.nomor_seri_awal) like :keyword1 
	or upper(b.nomor_seri_akhir) like :keyword2)

-- name: GetChequeList-filter-StatusBuku
and (upper(b.status_buku) = :status_buku)
...
insert into %[1]s.notadebetinternal 
		...
		values (%[1]s.seq_notadebetinternal.nextval, :JENIS_WARKAT, :ID_BUKUWARKAT, :NOMOR_SERI, :STATUS_WARKAT, current_timestamp, :USER_PENGUBAH, :NOMOR_REKENING, :USED_COUNTER)
...
and rt.status_rekening = :status_rekening

-- name: GetDepList-filter-Keyword
and ((upper(rt.nama_rekening) like :keyword1 or upper(rt.nomor_rekening) like :keyword2))

-- name: GetDepList-filter-NomorNasabah
and rl.nomor_nasabah = :nomor_nasabah
...
select rt.nomor_rekening, rt.nama_rekening, rt.kode_cabang, c.nama_cabang, nvl(rl.alamat_kirim, 'N') alamat_kirim, n.jenis_nasabah,
...
INSERT INTO %[1]s.histcetakbilyetdepo (ID_HISTORI, NOMOR_REKENING, USER_CETAK, USER_OVERRIDE, TANGGAL_CETAK)
		VALUES (%[1]s.seq_histcetakbilyetdepo.nextval, :NOMOR_REKENING, :USER_INPUT, :USER_OTORISASI, SYSDATE)
...
select m.no_nasabah as nomor_nasabah,
			...
			m.nomor_npwp,
			m.is_dhib,
			case when m.is_dhn='T' then m.is_dhn || ' #' || nvl(m.ctr_match_dhn,0) else m.is_dhn end as is_dhn,
			m.ctr_match_dhn,
			d.no_dhn,
			d.batas_sanksi,
...
SELECT
		...
		WHERE f.norek_finaccount = :nomor_rekening1
		) END as coll_count 
	FROM %[1]s.finaccount fa, %[1]s.finproduct fp 
	WHERE 
	fa.nomor_rekening = :nomor_rekening2 
	AND fa.product_code = fp.product_code

-- name: GetNeedFindataComplete-main
SELECT 
		nvl(need_findata_complete, 'F') as need_findata_complete
...
SELECT 
		nvl(count(*), 0) as cust_data_count 
	FROM %[1]s.fincustadditionaldata 
	WHERE nomor_nasabah = :nomor_nasabah 

-- name: GetAccDataCount-main
SELECT 
		nvl(count(*), 0) as acc_data_count 
...
ID_HOLD_DANA = :ID_HOLD_DANA

-- name: GetHoldDanaRekening-filter-NomorReferensiHold
NOMOR_REFERENSI = :NOMOR_REFERENSI_HOLD
...
INSERT INTO %[1]s.finpaymentscheddetail (
			...
			%[1]s.seq_finpaymentscheddetail.nextval
			, :ID_SCHEDULE
			, :SCHED_DATE
			, :SEQ_NUMBER
			, :PRINCIPAL_AMOUNT
...
SELECT 
		...
		case
			...
			else t.saldo - p.saldo_minimum - nvl(l.saldo_ditahan, 0)
		end as saldo_efektif,
		t.status_rekening,
		case 
...
select rl.nomor_nasabah, n.jenis_nasabah, rl.nomor_rekening, rt.nama_rekening, rt.saldo, rt.kode_cabang, c.nama_cabang,
			...
			rt.saldo - (nvl(rl.saldo_float, 0.0) + nvl(rl.saldo_ditahan, 0.0) + nvl(p.saldo_minimum, 0.0)) as saldo_efektif,
			case 
			...
			rl.jenis_register_layanan, rl.status_restriksi, nvl(rl.kode_multicifrelation, rd.reference_code) AS kode_multicifrelation
		from %[1]s.rekeningliabilitas rl
			inner join %[1]s.rekeningtransaksi rt on rl.nomor_rekening = rt.nomor_rekening
			inner join %[1]s.produk p on p.kode_produk = rl.kode_produk
			inner join %[1]s.nasabah n on n.nomor_nasabah = rl.nomor_nasabah 
...
select l.id_individu, i.nama_lengkap, i.tempat_lahir, to_char(i.tanggal_lahir, 'yyyy-mm-dd') as tanggal_lahir, i.status_perkawinan, 
			...
			nvl(i.kode_pekerjaan, kp.reference_desc) as kode_pekerjaan, i.telepon_hp_nomor, i.jenis_identitas_lain, i.nomor_identitas_lain, i.status_pep, 
			l.kode_sumber_dana, l.is_aktif, l.keterangan, i.jenis_kelamin, nvl(i.kode_negara_asal, kn.reference_desc) as kode_negara_asal,
...
and t.nomor_referensi = :nomor_referensi

-- name: GetTrxList-filter-KodeCabangTransaksi
and t.kode_cabang_transaksi = :kode_cabang_transaksi
...
where ht.tanggal_transaksi >= to_date(:tanggal_awal, 'yyyy-mm-dd') and ht.tanggal_transaksi < to_date(:tanggal_akhir, 'yyyy-mm-dd')

-- name: GetHistTrxList-filter-NomorReferensi
and ht.nomor_referensi = :nomor_referensi

-- name: GetHistTrxList-filter-KodeCabangTransaksi
and ht.kode_cabang_transaksi = :kode_cabang_transaksi
...
insert into %[1]s.detiltransaksi
		(id_detil_transaksi, tanggal_transaksi, jenis_mutasi, nilai_mutasi, keterangan, kode_cabang, kode_valuta, jenis_detil_transaksi,
			...
			flag_code, kode_rc)
		values (%[1]s.seq_detiltransaksi.nextval, :TANGGAL_TRANSAKSI, :JENIS_MUTASI, :NILAI_MUTASI, :KETERANGAN, :KODE_CABANG, :KODE_VALUTA, :JENIS_DETIL_TRANSAKSI,
			:NILAI_KURS_MANUAL, :SALDO_AWAL, :NILAI_EKUIVALEN, :NOMOR_REFERENSI, :KODE_ACCOUNT, :KODE_VALUTA_RAK, :KODE_JURNAL, :ID_TRANSAKSI, :NOMOR_REKENING, 
			:JENIS_TRANSAKSI_LIABILITAS, :ID_PARAMETER_TRANSAKSI, :FLAG_PROSES, :IS_CREATE_REMITTANCE, :SEQUENCE, :KODE_TX_CLASS, :NOMOR_SERI_WARKAT,
			:FLAG_CODE, :KODE_RC)
...
SELECT %v.seq_val_transaksi_rekening.nextval FROM dual
...
select %[1]s.seq_val_transaksi_umum.nextval from dual
...
merge into %[1]v.validasi_transaksi_umum tmp
		using %[2]v.rekeningtransaksi r 
		on (r.nomor_rekening = tmp.nomor_rekening)
		when matched then update set 
...
merge into %[1]v.validasi_transaksi_umum tmp
		using %[2]v.rekeningliabilitas r 
		on (r.nomor_rekening = tmp.nomor_rekening)
		when matched then update set 
		...
		where id_session = :id_session and kode_tipe = '002'
...
SELECT %[1]s.seq_val_transaksi_payroll.nextval from dual
...
DELETE FROM  %[1]s.VALIDASI_TRANSAKSI_PAYROLL TMP WHERE ID_SESSION= :ID_SESSION
...
UPDATE %[1]s.validasi_transaksi_umum tmp
		...
		WHERE ID_SESSION = :ID_SESSION
		AND JENIS_MUTASI ='C'
		AND EXISTS (SELECT 1 FROM %[2]s.rekeningliabilitas n WHERE n.NOMOR_rekening=tmp.NOMOR_rekening and nvl(n.is_blokir_kredit,'F') ='T')
...
UPDATE %[1]s.validasi_transaksi_umum tmp
		...
		WHERE ID_SESSION = :ID_SESSION
		AND JENIS_MUTASI ='D'
		AND EXISTS (SELECT 1 FROM %[2]s.rekeningliabilitas n WHERE n.NOMOR_rekening=tmp.NOMOR_rekening and nvl(n.is_blokir_debet,'F') ='T')
...
delete from  %[1]s.validasi_transaksi_umum tmp where id_session= :ID_SESSION
...
insert into %[1]s.transfer
		...
		values
		(:JENIS_TRANSFER, :NAMA_PENERIMA, :NAMA_PENGIRIM, :NOMOR_REKENING_PENERIMA, %[1]s.seq_transfer.nextval, :KODE_BANK, :ID_DETIL_TRANSAKSI, :ID_TRANSAKSI, 
			:NOMOR_REKENING_PENGIRIM, :STATUS_PENGIRIM, :JENIS_PENDUDUK, :KODE_ACCOUNT_TITIPAN, :KODE_CABANG_TITIPAN, :JENIS_PENGIRIM, :JENIS_PENERIMA, 
			:JENISPENDUDUK_PENGIRIM, :JENISPENDUDUK_PENERIMA, :NOIDENTITAS_PENGIRIM, :NOIDENTITAS_PENERIMA, :ALAMAT_PENGIRIM, :ALAMAT_PENERIMA, 
			:JENIS_CHANNEL, :SANDIKOTA_PENGIRIM, :SANDIKOTA_PENERIMA, :KODE_TX_SKN, :KODE_TX_RTGS, :NOMOR_REFERENSI_RETUR, :JENISNASABAH_PENGIRIM, 
...
select rt.nama_rekening
		...
		, rt.kode_jenis
		, rt.saldo - (nvl(rl.saldo_float, 0.0) + nvl(rl.saldo_ditahan, 0.0) + nvl(p.saldo_minimum, 0.0)) as saldo_efektif
		, rt.saldo - (nvl(rl.saldo_float, 0.0) + nvl(rl.saldo_ditahan, 0.0)) as saldo_override
		, p.kode_produk
		, p.nama_produk
...
select 
		va.accountname
		, nvl(vap.admcosttype, 'F') as admcosttype
		, nvl(vap.admcost, 0) as admcost
		, vap.paymenttype
		, vap.nomor_rekening
		, rt.kode_cabang
...
-- name: GetIDIdv-main
select %[1]s.seq_individu.nextval as id_idv FROM dual

-- name: GetIDRegLayanan-main
select %[1]s.seq_registerlayanan.nextval as id from dual
...
select %[1]s.seq_transaksi.nextval as id_transaksi from dual

-- name: GetTransaksiDetailID-main
select %[1]s.seq_detiltransaksi.nextval as id_detil_transaksi from dual

-- name: GetCustomIdgen-main
select idcode,
case 
    when last_id is null then 0
    else (%[1]s.seq_custom_idgen.nextval - last_id + 1) 
end as last_idgen 
...
values (:IDNAME, :IDCODE, %[1]s.seq_custom_idgen.nextval, :LOCKED)
...
select * from %[1]s.glaccount where nomor_rekening = :nomor_rekening
...
select ubitems.fl_journal, %[1]s.seq_channeljournalitemno.nextval, :FL_ACCOUNT1,  
...
and (acc.account_code like :keyword or acc.account_name like :keyword2)
...
(1 = :all_cabang or branch_code = :kode_cabang )
    AND journal_date >= TO_DATE(:tgl_awal, 'YYYY-MM-DD') 
	AND journal_date < TO_DATE(:tgl_akhir, 'YYYY-MM-DD')

-- name: GetGLJurnal-filter-JurnalNoGTZero
and journal_no LIKE ( '%%' || :journal_no || '%%' )
...
values(%[1]s.seq_historiubahdata.nextval, :NOMOR_REKENING, current_timestamp, :USER_UBAH, :NAMA_CLASS, :KETERANGAN_UBAH)
...
values (:FL_JOURNAL, %[1]s.seq_channeljournalitemno.nextval, :FL_ACCOUNT, :AMOUNT_DEBIT, :AMOUNT_CREDIT, :DESCRIPTION, :BRANCH_CODE, :VALUTA_CODE, :USERID_CREATE, :JOURNALITEMSTATUS, 
...
using %[1]s.transaksi upd
on (upd.id_transaksi = target.id_transaksi and %[2]s)
when matched then update set journal_no = upd.journal_no

--name: MergeJournalNumber-filter-IdTransaksiNotZero
target.id_transaksi = :ID_TRANSAKSI
...
select ptj.journal_no, %[1]s.seq_journalitem.nextval, dt.kode_account,
    case 
        when dt.jenis_mutasi = 'D' then dt.nilai_mutasi
        else 0.0
    end as amount_debit,
...
select 
    ubitems.fl_journal, %[1]s.seq_journalitem.nextval, :ACC_CODE_RAK1, ubitems.amount_credit, ubitems.amount_debit, 'BALANCING RAK OTOMATIS', ubitems.branch_code, ubitems.valuta_code, null, 
...
using (
    ...
    from %[1]s.pendingtransactionjournal ptj
    where %[2]s
) upd on (upd.id_transaksi = t.id_transaksi)
when matched then
update set is_sudah_dijurnal = 'T'
...
select %[1]s.seq_notadebitoutwardgroup.nextval as id_group from dual

-- name: GetIdNotadebitoutward-main
select %[1]s.seq_notadebitoutward.nextval as id_notadebitoutward from dual
...
WHEN MATCHED THEN
    UPDATE
    SET
        saldo_float = nvl(q.total_saldo_float, 0.0)
    WHERE
...
values
(%[1]s.seq_multiciflink.nextval, :NOMOR_REKENING, :NOMOR_NASABAH)
...
set status_warkat = :STATUS_WARKAT,
    ...
    used_counter = nvl(used_counter, 0) + 1
where nomor_seri = :NOMOR_SERI
    and jenis_warkat = :JENIS_WARKAT
    and nomor_rekening = :NOMOR_REKENING
...
values(%[1]s.seq_histcetaksaldopassbook.nextval, :ID_REGISTER, SYSDATE, :USER_INPUT, :SALDO_CETAK_TERAKHIR, :NOMOR_BUKU_TERAKHIR, :HALAMAN_CETAK_TERAKHIR, :BARIS_CETAK_TERAKHIR, :ALASAN, :USER_OTORISASI)
...
select regp.id_register, rt.nomor_rekening, rt.nama_rekening, rt.kode_cabang, c.nama_cabang, regp.nomorregisterbuku_aktif, nvl(rl.alamat_kirim,'N') alamat_kirim, 
...
values (%[1]s.seq_historycetaknamapassbook.nextval, :USER_OVERRIDE, :NOMOR_REKENING, :USER_INPUT, sysdate, :TANGGAL_SISTEM, :NOMOR_REGISTER_BUKU, :NOMOR_REGISTER_BUKU_LAMA, 
    :ALASAN_CETAK, :KODE_CABANG_INPUT, :IP_INPUT
)
...
select %[1]s.seq_infocetakpassbook.nextval
from dual
...
and i.id_cetak_passbook >= :id_cetak_awal
	and i.id_cetak_passbook <= :id_cetak_akhir

-- name: GetCollectionDataTrx-filter-AllDataTrue2
and i.id_cetak_passbook >= :id_cetak_awal2
	and i.id_cetak_passbook <= :id_cetak_akhir2
...
select p.kode_produk, p.nama_produk, p.nisbah_bonus_dasar, p.is_backdated, nvl(p.is_dapat_bagi_hasil,'F') as is_dapat_bagi_hasil, nvl(p.tarif_pajak, 0.0) as tarif_pajak, nvl(p.presentase_zakat_bagi_hasil, 0.0) as presentase_zakat_bagi_hasil, p.kode_valuta, nvl(pd.masa_perjanjian, 0) as masa_perjanjian, nvl(pd.periode_perjanjian, 'B') as periode_perjanjian, p.is_kena_biayalayananumum, p.is_biaya_atm, p.jenis_produk, is_biaya_rekening_dormant, is_biaya_saldo_minimum, is_blokir_debet, is_blokir_kredit, is_cetak_nota, is_status_passbook,  is_tidak_dormant
...
VALUES 
(%[1]s.seq_glinterface.nextval, :KODE_INTERFACE, :DESKRIPSI, :KODE_ACCOUNT)
...
VALUES 
(%[1]s.seq_tieringitem.nextval ,:TIERINGID, :LOWERLIMITVALUE , :UPPERLIMITVALUE , :AMOUNT1)
...
VALUES 
(%[1]s.seq_ketentuan.nextval, :KODE_LIMIT , :JENIS_LIMITASI , :PERIODE , :JUMLAH_TRANSAKSI , :NILAI_TRANSAKSI, :KODE_PRODUK)
...
select %[1]s.seq_tieringplan.nextval as id_tiering_nisbahbonus
from dual

-- name: GetSeqIdTieringBiayaAdmin-main
select %[1]s.seq_tieringplan.nextval as id_tiering_biayaadm
from dual
...
FROM %[1]s.produk p WHERE p.KODE_PRODUK = :kode_produk

-- name: GetKodeProdukDepExist-main
SELECT p.KODE_PRODUK 
FROM %[1]s.PRODUKDEPOSITO p WHERE p.KODE_PRODUK = :kode_produk
...
DELETE FROM %[1]s.TIERINGITEM t WHERE t.ITEMID = :itemid

-- name: DelLimitProd-main
DELETE FROM %[1]s.KETENTUAN k WHERE k.ID_KETENTUAN = :id_ketentuan
...
, n.is_wic
, n.tanggal_buka
, nvl(qr.pembiayaan, 0) as pembiayaan
, nvl(qr.tahapan, 0) as tahapan
, nvl(qr.deposito, 0) as deposito
, nvl(qr.trib, 0) as trib
, nvl(qr.giro, 0) as giro
, nk.nama_perusahaan
, nk.nomor_npwp
, nk.nomor_siupp
, nk.tanggal_siupp
...
values (
    %[1]s.seq_update_ref_sched.nextval, :ACTION_SCHED, :NOMOR_REKENING, :NOMOR_NASABAH, current_timestamp, :USER_UPDATE, :STATUS_UPDATE, :KEYINT
)
...
select p.kode_produk, p.nama_produk, p.is_onbalancesheetmode, p.saldo_minimum_tidak_aktif, p.jumlah_hari_jadi_tidak_aktif, p.saldo_minimum, 
    ...
    p.is_status_passbook, p.kolektibilitas, nvl(p.is_dapat_bagi_hasil, 'F') as is_dapat_bagi_hasil, p.is_tidak_dormant, p.tanggal_acuan_dormant, 
...
select p.kode_produk, 
    ...
    p.nama_valuta,
    nvl(p.is_dapat_bagi_hasil,'F') as is_dapat_bagi_hasil,
    nvl(p.tarif_pajak, 0.0) as tarif_pajak,
    nvl(p.presentase_zakat_bagi_hasil, 0.0) as presentase_zakat_bagi_hasil,
    p.kode_valuta,
...
select p.kode_produk, p.nama_produk, p.is_onbalancesheetmode, p.saldo_minimum_tidak_aktif, p.jumlah_hari_jadi_tidak_aktif, p.saldo_minimum, 
    ...
    p.persentase_cadangan, p.order_mb, p.is_biaya_materai, p.is_override_biayaatm, p.id_rencanabagihasildefault, p.id_biayalayananumum, 
    p.id_tiering_biayaadm, p.id_tiering_nisbahbonus, p.id_asuransi, pc.kode_cabang,
    nvl(pd.masa_perjanjian, 0) as masa_perjanjian, nvl(pd.periode_perjanjian, 'B') as periode_perjanjian
from %[1]s.coreproduct c
    inner join %[1]s.produk p on p.kode_produk = c.productcode
    inner join %[1]s.produkcabang pc on pc.kode_produk = p.kode_produk
    left join %[1]s.produkdeposito pd on pd.kode_produk = p.kode_produk
...
(rl.nomor_rekening LIKE :nomor_rekening1 or upper(rt.nama_rekening) LIKE :nomor_rekening2)
...
(upper(l.nomor_rekening) LIKE :nomor_rekening1 or upper(rt.nama_rekening) LIKE :nomor_rekening2)
...
select 
    case 
        when last_id is null then 0 
        else (%[1]v.seq_doicr_ref.nextval - last_id + 1) 
    end as last_idgen 
...
(idname, idcode, last_id, locked)
values 
('DOICREDITREF', :id_code , %[1]v.seq_doicr_ref.nextval, 'F') 
...
select %[2]v.seq_trx_record.nextval, %[2]v.seq_trx_status_history.nextval, dt.id_detil_transaksi, t.kode_cabang_transaksi, current_timestamp, t.user_input, :sandi_bi, r.nomor_rekening_pengirim, substr(r.nama_pengirim,1,70), 
...
(record_id, record_detail_id, status_history_id, id_detil_transaksi, branch_code, created_time, userid, ori_sandi_bank, ori_nomor_rekening, ori_nama_pengirim, 
    ...
    queue_branchcode_outgoing, id_transfer)
select %[2]v.seq_rttransaction.nextval, %[2]v.seq_rttransferdetail.nextval, %[2]v.seq_rtstatushistory.nextval, dt.id_detil_transaksi, t.kode_cabang_transaksi, current_timestamp, t.user_input, c.sandi_bi, r.nomor_rekening_pengirim, substr(r.nama_pengirim,1,140), 
...
merge into %[2]v.doi_credit r
using %[3]v.transfer t
on (t.id_transfer = r.id_transfer)
when matched then update set 
...
(record_id, record_detail_id, status_history_id, id_detil_transaksi, branch_code, created_time, userid, ori_sandi_bank, ori_nomor_rekening, ori_nama_pengirim, 
    ...
    queue_branchcode_outgoing, id_transfer)
select
    %[1]v.seq_trxrecord.nextval, %[1]v.seq_rttransferdetail.nextval, %[1]v.seq_rtstatushistory.nextval, dt.id_detil_transaksi, t.kode_cabang_transaksi, current_timestamp, t.user_input, :sandi_bi, r.nomor_rekening_pengirim, substr(r.nama_pengirim,1,140),
...
merge into %[1]v.rttransaction s
using (
...
merge into %[1]v.rtsingletransfer s
using (
    select tmp.record_detail_id, r.*
    from %[2]v.remittance_candidate tmp
        inner join %[3]v.transfer r on r.id_transfer = tmp.id_transfer
) q on (q.record_detail_id = s.transdetail_id)
when matched then update
set resident_status = (
...
and lower(nama_report) like lower(:keyword1) or lower(kode_report) like lower(:keyword2)
...
select
    ...
    rl.nomor_nasabah,
    rl.kode_program,
    nvl(rl.is_join_account,'F') as is_join_account,
...
select
    ...
    n.User_Input,
    upper(nvl(o.User_Otorisasi, n.user_otorisasi)) as user_otorisasi,
    ...
    case when nvl(p2.rule_flag, p3.rule_flag) = 'T' then 'Tidak' when p1.rule_flag = 'Y' then 'Ya' else '-' end FLAG_NRT,
...
select
    ...
    ND.Nomor_Rekening,
    RT.Nama_Rekening,
    nvl(qb.Nama_Singkat, m.nama_member) as Nama_Bank,
    T.User_Input,
    T.User_Otorisasi,
    DT.Id_Detil_Transaksi
from
...
from
    %[1]v.rekeningtransaksi RT
    left outer join table(get_dailybalancerek_bydate(to_date(:TANGGAL_AKHIR, 'yyyy-mm-dd'))) db 
...
FROM
%[1]v.REKENINGTRANSAKSI RT
LEFT OUTER JOIN table(GET_DAILYBALANCEREK_BYDATE(TO_DATE(:TANGGAL_AKHIR, 'YYYY-MM-DD') + 1)) db
    on (db.nomor_rekening = rt.nomor_rekening and db.balance_field='saldo')
INNER JOIN %[1]v.REKENINGCUSTOMER RC ON RT.Nomor_Rekening = RC.Nomor_Rekening
INNER JOIN %[1]v.REKENINGLIABILITAS RL ON RC.Nomor_Rekening = RL.Nomor_Rekening
INNER JOIN %[1]v.PRODUK  P ON RL.Kode_Produk = P.Kode_Produk
...
SELECT
    nvl(kode_jenis, 'NOREK') AS kode_jenis,
...
SELECT 
    ...
    t.id_transaksi,    
    t.is_reversed,    
    nvl(b.Nama_Lengkap, m.nama_member) as nama_bank,
    t.user_overide,
    infot.nomor_rekening_debet,
    infot.nominal_biaya,
    enum.enum_description,
...
SELECT 
    h.batch_no, 
    ts.session_date,  
	db.ori_kli_code || ' - ' || nvl(b.Nama_Lengkap, m.nama_member) as sandi_bank,
...
SELECT 
    ...
    d.nilai_mutasi,    
    rec.branch_code,
	cr.ori_kli_code|| ' - ' || nvl(b.Nama_Lengkap, m.nama_member) as bank_pengirim,
    nvl(b.Nama_Lengkap, m.nama_member) as nama_bank,
    1 AS data_count
  FROM
    %[2]v.DIE_CREDIT cr 
...
VALUES (%[1]s.seq_custaccountimage.nextval, :IMAGETAG, :REMARK, :ACCOUNTNO, :ID_DATA, :TAGSEQUENCE, 1)
